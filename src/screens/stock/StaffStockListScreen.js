...
import { ArrowLeft, Bike, CircleX, Filter, Palette, Package, Search } from 'lucide-react-native';
...
const StaffStockListScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    return unsubscribe;
  }, [navigation, selectedMotorbikeId, selectedColorId]);

  const loadInitialData = async () => {
    await Promise.all([loadMotorbikes(), loadColors()]);
    await loadStocks();
  };
  ...
  const loadColors = async () => {
    try {
      ...
      const agencyId = parseInt(user.agencyId, 10);
      ...
      if (response.success) {
        ...
        await Promise.all(
          stocksData
            .filter((stock) => stock.colorId)
            .map(async (stock) => {
              if (colorIdMap.has(stock.colorId)) {
                return;
              }

              try {
                const detailResponse = await agencyStockService.getAgencyStockDetail(stock.id);
                if (detailResponse.success && detailResponse.data?.color) {
                  const color = detailResponse.data.color;
                  colorIdMap.set(stock.colorId, {
                    id: stock.colorId,
                    colorType: color.colorType || `Color ${stock.colorId}`,
                  });
                } else {
                  colorIdMap.set(stock.colorId, {
                    id: stock.colorId,
                    colorType: `Color ${stock.colorId}`,
                  });
                }
              } catch (error) {
                console.error(`Error loading color info for colorId ${stock.colorId}:`, error);
                colorIdMap.set(stock.colorId, {
                  id: stock.colorId,
                  colorType: `Color ${stock.colorId}`,
                });
        ...
        setColors(Array.from(colorIdMap.values()));
      }
    } catch (error) {
  ...
  const loadStocks = async (withLoader = true) => {
    try {
      if (withLoader) {
        setLoading(true);
      }
  ...
  const renderStockCard = ({ item: stock }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.stockCardShadow}
        activeOpacity={0.85}
        onPress={() => handleViewDetail(stock)}
      >
        <View style={styles.stockCard}>
          <View style={styles.stockImageContainer}>
            <Image source={{ uri: stockImage }} style={styles.stockImage} resizeMode="cover" />
            <View style={[styles.statusBadge, isInStock ? styles.statusBadgeSuccess : styles.statusBadgeError]}>
          ...
          <View style={styles.modalBody}>
            <View style={styles.filterSection}>
              ...
              <View style={styles.filterOptions}>
                <TouchableOpacity
                  style={[styles.filterOption, !selectedMotorbikeId && styles.selectedFilterOption]}
            ...
            <View style={styles.filterSection}>
              ...
              <View style={styles.filterOptions}>
                <TouchableOpacity
                  style={[styles.filterOption, !selectedColorId && styles.selectedFilterOption]}
                ...
                {colors.length === 0 && <Text style={styles.modalEmptyText}>No colors available</Text>}
              </View>
            </View>
          </View>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerTitle}>
            ...
            <Text style={styles.headerSubtitle}>Browse available inventory</Text>
          </View>
      ...
      <View style={styles.content}>
        {loading ? (
          ...
          <FlatList
            ...
            refreshControl={
              <RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={PRIMARY_ACCENT} colors={[PRIMARY_ACCENT]} />
            }
            ListHeaderComponent={
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyTitle}>{searchQuery ? 'No results found' : 'No stock items'}</Text>
                <Text style={styles.emptySubtitle}>
                  {searchQuery ? 'Try adjusting your search' : 'No stock items available'}
                </Text>
              </View>
            }
          />
