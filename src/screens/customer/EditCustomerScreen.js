...
const EditCustomerScreen = ({ navigation, route }) => {
    ...
    try {
      ...
      if (data) {
        ...
        setFormData({
          ...
          dob: data.dob ? formatDate(dobDate) : '', // Format date
        });
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      const updateData = {
        ...formData,
        dob: formData.dob ? new Date(formData.dob).toISOString() : customer.dob,
      };

      const updatedCustomer = await customerManagementService.updateCustomer(customerId, updateData);

      if (updatedCustomer) {
        showSuccess('Success', 'Customer information updated successfully!');
        setTimeout(() => {
          navigation.goBack();
        }, 1500);
      } else {
      ...
      showError('Error', error.message || 'Failed to update customer. Please try again.');
    } finally {
      setSaving(false);
    }
  };
