...
import { ArrowLeft, NotepadText, Plus, Search } from 'lucide-react-native';
...
const DEFAULT_IMAGES = {
  modely: require('../../../assets/images/banner/banner-modely.png'),
  modelx: require('../../../assets/images/banner/Banner-modelx.png'),
  modelv: require('../../../assets/images/banner/banner-modelv.png'),
};

const DealerManagerQuotationScreen = ({ navigation }) => {
  const { user } = useAuth();
  ...
  const [selectedTabType, setSelectedTabType] = useState('AT_STORE');
  ...
  const agencyId = useMemo(() => {
    ...
    const parsed = Number(user.agencyId);
    return Number.isNaN(parsed) ? null : parsed;
  }, [user?.agencyId]);

  const getVehicleImage = (vehicleImage) => {
    if (!vehicleImage) {
      return DEFAULT_IMAGES.modely;
    }
    ...
    const imageMap = {
      'banner-modely.png': DEFAULT_IMAGES.modely,
      'banner-modelx.png': DEFAULT_IMAGES.modelx,
      'banner-modelv.png': DEFAULT_IMAGES.modelv,
    };

    return imageMap[vehicleImage] || DEFAULT_IMAGES.modely;
  };
  ...
  const loadQuotations = useCallback(async () => {
    ...
    try {
      if (!agencyId) {
        showErrorRef.current?.('Missing information', 'Could not find an agencyId for your account.');
        setAllQuotations([]);
        return;
      }

      const response = await quotationService.getQuotationsByAgency(agencyId, { limit: 1000 });
      ...
      if (response.success) {
        ...
        setAllQuotations(sorted);
        loadQuotationDetails(sorted);
      } else {
        setAllQuotations([]);
        showErrorRef.current?.('Error', response.error || 'Unable to load quotation list.');
      }
    } catch (error) {
      console.error('Error loading quotations:', error);
      showErrorRef.current?.('System error', 'Failed to load quotation list.');
      setAllQuotations([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadQuotationDetails = async (quotes) => {
    const quotationsWithDetails = await Promise.all(
      quotes.map(async (quote) => {
        try {
          const detailResponse = await quotationService.getQuotationById(parseInt(quote.id, 10));
          if (detailResponse.success && detailResponse.data) {
            const detail = detailResponse.data;
            let vehicleImage = 'banner-modely.png';
            ...
            return {
              ...
              vehicleModel: detail.motorbike?.name || 'Unknown model',
              vehicleImage,
            };
          }
          return quote;
    ...
    filterQuotations(quotationsWithDetails);
  };
  ...
  const filterQuotations = (source = allQuotations) => {
    let filtered = source;
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((quotation) =>
        ...
        quotation.quoteCode?.toLowerCase().includes(query)
      );
    }

    if (selectedFilter !== 'all') {
      filtered = filtered.filter(
        (quotation) => quotation.status?.toUpperCase() === selectedFilter.toUpperCase()
      );
    }
  ...
  const handleQuotationPress = (quotation) => {
    navigation.navigate('DealerManagerQuotationDetail', {
      quotation,
    });
  };
  ...
  const getQuotationsByTab = () => {
    return allQuotations.filter(
      (q) => q.type?.toUpperCase() === selectedTabType.toUpperCase()
    );
  };
  ...
  const filterOptions = [
    { key: 'all', label: 'All', count: quotationsByTab.length },
    {
      key: 'DRAFT',
      label: 'Draft',
      count: quotationsByTab.filter((q) => q.status === 'DRAFT').length,
    },
    {
      key: 'ACCEPTED',
      label: 'Accepted',
      count: quotationsByTab.filter((q) => q.status === 'ACCEPTED').length,
    },
    {
      key: 'REJECTED',
      label: 'Rejected',
      count: quotationsByTab.filter((q) => q.status === 'REJECTED').length,
    },
    {
      key: 'EXPIRED',
      label: 'Expired',
      count: quotationsByTab.filter((q) => q.status === 'EXPIRED').length,
    },
  ];
      ...
      >
        <View style={styles.cardHeader}>
          <View style={styles.quotationInfo}>
            <Text style={styles.quotationId}>#{quotation.quoteCode || quotation.id}</Text>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <View style={styles.headerTitle}>
            ...
            <Text style={styles.headerSubtitle}>Dealer Manager</Text>
          </View>
        </View>
      </View>
...
export default DealerManagerQuotationScreen;
