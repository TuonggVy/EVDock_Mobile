...
const CreateDealerStaffScreen = ({ navigation, route }) => {
  const { agencyId, roleId } = route.params || {};
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await staffService.createDealerStaff(payload);

      if (res?.success) {
        showSuccess('Success', res?.message || 'Dealer staff created successfully!');
        setTimeout(() => {
          navigation.goBack();
        }, 1500);
      } else {
        showError('Error', res?.error || 'Unable to create dealer staff');
      }
    } catch (error) {
      console.error('Error creating dealer staff:', error);
      showError('Error', 'Unable to create dealer staff');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Create Dealer Staff</Text>
...
export default CreateDealerStaffScreen;
